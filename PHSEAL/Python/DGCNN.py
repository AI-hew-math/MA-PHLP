...
from pytorch_DGCNN.util import cmd_args, load_data
...
class Classifier(nn.Module):
    def __init__(self, regression=False):
        ...
        if cmd_args.graph_feature:
            if cmd_args.multi_angle:
    ...
    def forward(self, batch_graph):
        ...
        if cmd_args.graph_feature:
            if cmd_args.multi_angle:
                graph_features = torch.zeros(embed.size(0), int((cmd_args.h+3)*(cmd_args.h)*(1/2)*36))
            else:
                graph_features = torch.zeros(embed.size(0), 36)
            for i in range(embed.size(0)):
                graph_features[i] = batch_graph[i].graph_features
            graph_features = graph_features.cuda()
            out_tda= self.linear(graph_features)
            ...
            embed = torch.cat((embed*alpha[0], out_tda*alpha[1]),dim=1)
    ...
    def output_features(self, batch_graph):
        ...
        if cmd_args.graph_feature:
            if cmd_args.multi_angle:
                graph_features = torch.zeros(embed.size(0), int((cmd_args.h+3)*(cmd_args.h)*(1/2)*128)) 
            else:
                graph_features = torch.zeros(embed.size(0), 128)
            for i in range(embed.size(0)):
                graph_features[i] = batch_graph[i].graph_features
            graph_features = graph_features.cuda()
            embed = torch.concatenate([embed,graph_features],axis=1)
