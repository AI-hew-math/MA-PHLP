...
parser = argparse.ArgumentParser(description='Link Prediction with PHSEAL')
# general settings
...
# model settings
parser.add_argument('--graph-feature', type=str2none, default=None, help='if None, does not use persistent homology. \
                    Otherwise, compute persistent homology')
parser.add_argument('--multi-angle', type=str2bool, default=False, 
                    help='if True, extract multi angle sugraph when computing persistent homology. \
                    Elif False, it compute persistent homology on extracted subgraph')
...
cmd_args.feat_dim = int(max_n_label) + 1
cmd_args.attr_dim = 0
cmd_args.graph_feature = args.graph_feature
cmd_args.multi_angle = args.multi_angle
cmd_args.h = args.hop
if node_information is not None:
...
val_num = int(0.05556 * len(train_graphs))
...
if cmd_args.printAUC:
    if args.graph_feature is None:
        with open(args.data_name+'_SEAL_auc_results.txt', 'a+') as f:
            f.write('Seed'+str(seed)+' score: '+str(test_loss[2]) + '\n')
    else:
        if args.multi_angle:
            with open(args.data_name+'_MultiPHSEAL_auc_results.txt', 'a+') as f:
                f.write('Seed'+str(seed)+' score: '+str(test_loss[2]) + '\n')
        else:
            with open(args.data_name+'_PHSEAL_auc_results.txt', 'a+') as f:
                f.write('Seed'+str(seed)+' score: '+str(test_loss[2]) + '\n')
