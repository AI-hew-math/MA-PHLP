...
class PH_LinkPred(MessagePassing):
    def __init__(self, in_channels: int, hidden_channels: int, heads: int = 1, tda_vector_size: int = 72, \
                 walk_len: int = 6, drnl: bool = False, z_max: int =100, MSE: bool=True):
        super(PH_LinkPred, self).__init__()
        ...
        L=walk_len*5+1
        tda_output_size = L*heads
        self.linear = torch.nn.Linear(tda_vector_size, tda_output_size)
        # self.act= nn.Sigmoid()
        self.act= nn.Softmax(dim=1)
    
        self.classifier = MLP(L*heads + tda_output_size,MSE=MSE)
        self.alpha = torch.nn.Parameter(torch.zeros(2))
