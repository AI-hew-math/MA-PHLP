...
def prepare_data(args):
    ...
    if args.PH:
        print('Make train TDA vector')
        train_graphs = parmap.starmap(get_multi_PI, [[subgph] for subgph in train_graphs], args=args, pm_pbar=True, pm_processes=args.num_cpu)

        print('Make valid TDA vector')
        val_graphs = parmap.starmap(get_multi_PI, [[subgph] for subgph in val_graphs], args=args, pm_pbar=True, pm_processes=args.num_cpu)

        print('Make test TDA vector')
        test_graphs = parmap.starmap(get_multi_PI, [[subgph] for subgph in test_graphs], args=args, pm_pbar=True, pm_processes=args.num_cpu)
